...
const MyBooking = () => {
    ...
    const handleCancelBooking = (id) => {
        axios
            ...
            .then(() => {
                setBookingCar((prev) => prev.filter((booking) => booking._id !== id));
            })
            .catch((err) => {
    ...
    return (
<div className="p-6">
  <h1 className="text-3xl font-bold text-center mb-8 flex items-center justify-center gap-4">My Bookings: <span className="bg-green-500 px-10 py-1 rounded-full text-white">{bookingCar.length}</span> </h1>
  ...
  {bookingCar?.length > 0 ? (
    ...
    >
      <div className="flex flex-col justify-center items-center space-y-4">
        <h2 className="text-5xl text-red-500">No Bookings Request Found</h2>
        <p className="text-gray-600">Looks like you haven't booked any cars yet!</p>
        <Link
          to="/add-car"
